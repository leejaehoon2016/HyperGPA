...
def get_ushcn_data_val(num_test_period, num_val_period, lstm_x_len, lstm_y_len, hyper_x_len, hyper_y_len, model_type, print_info = True, period_start = 0, period_end = None):
    df = pd.read_csv(f"../data/ushcn/ushcn.csv", header=0,index_col=0)
    df = df.sort_values(["state","year","month"])
    df = df[(df.year >= 1981) & (df.year <= 2012)]
    col = ['prcp', 'tavg', 'tmax', 'tmin'] # ['tavg', 'tmax', 'tmin'] #
    for s in df.state.unique():
        tmp = df.loc[df.state == s,col]
        tmp = (tmp - tmp.mean()) / tmp.std()
        df.loc[df.state == s,col] = tmp
    df.year = df.year - df.year.min()
    def change_year_month(x, stan=1):
        year, month = x
        month = (year % stan) * 12 + month
        year = year - (year % stan)
        return year, month
    tmp = df[['year','month']].apply(change_year_month,axis=1)
